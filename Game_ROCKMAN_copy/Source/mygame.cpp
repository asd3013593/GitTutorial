...
namespace game_framework {
	...
	void CGameStateRun::OnInit()  								// 遊戲的初值及圖形設定
	{
		...
		gamemap.LoadBitmap();
		role.LoadBitmap();
		practice.LoadBitmap(IDB_Background1);
		//
		// 當圖很多時，OnInit載入所有的圖要花很多時間。為避免玩遊戲的人
		//     等的不耐煩，遊戲會出現「Loading ...」，顯示Loading的進度。
		//
	...
	void CGameMap::RandomBoucingBall()
	{
		...
		for(int row = 0;row <4;row++)
			for (int col = 0; col < 5; col++)
			{
				...
				{
					InitializeBoucingBall(ini_index, row, col);
					ini_index++;
				}
			}
	}
	void CGameMap::InitializeBoucingBall(int ini_index, int row, int col)
	{
	...
	{
		animation.AddBitmap(IDB_ROCKSTAND, RGB(255, 255, 255));
		
		runRightAnimation.AddBitmap(IDB_RUNRIGHT, RGB(255, 255, 255));
		...
		jumpAnimation.AddBitmap(IDB_JUMP_1,RGB(255,255,255));
	...
	void CRoleRun::LoadBitmap()
	{
		animation.AddBitmap(IDB_RUNRIGHT, RGB(255, 255, 255));
	}
