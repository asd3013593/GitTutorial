...
namespace game_framework {
	...
	void CGameStateRun::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
	{
		...
		for (int i = 0; i < 3; i++) {
			if (role.HavingBullet(i) && nChar == KEY_Z)
			{
				role.SetMovingRight(true);
			}
		}
	...
	void CRole::Initialize()
	{
		...
		const int nY = 430;
		x = nX;  y = nY;
		for (int i = 0; i < 3; i++) bulletIndex[i] = 0;
		direction = isbulletMoving = isAttacking = isMovingDown = isMovingUp = isMovingLeft = isMovingRight = false;
		DisplayBullet();
	}
	void CRole::LoadBitmap()
	{
		for (int i = 0; i < 3; i++) bullet[i].AddBitmap(IDB_BULLET_1);
		standRightAnimation.AddBitmap(IDB_ROCKSTANDRIGHT, RGB(255, 255, 255));
		standLeftAnimation.AddBitmap(IDB_ROCKSTANDLEFT, RGB(255, 255, 255));
		runRightAnimation.AddBitmap(IDB_RUNNINGRIGHT_1, RGB(255, 255, 255));
		runRightAnimation.AddBitmap(IDB_RUNNINGRIGHT_2, RGB(255, 255, 255));
		runRightAnimation.AddBitmap(IDB_RUNNINGRIGHT_3, RGB(255, 255, 255));
		runRightAnimation.AddBitmap(IDB_RUNNINGRIGHT_1, RGB(255, 255, 255));
		runLeftAnimation.AddBitmap(IDB_RUNNINGLEFT_1, RGB(255, 255, 255));
		runLeftAnimation.AddBitmap(IDB_RUNNINGLEFT_2, RGB(255, 255, 255));
		runLeftAnimation.AddBitmap(IDB_RUNNINGLEFT_3, RGB(255, 255, 255));
		jumpRightAnimation.AddBitmap(IDB_JUMPRIGHT, RGB(255, 255, 255));
		jumpLeftAnimation.AddBitmap(IDB_JUMPLEFT, RGB(255, 255, 255));
		attackAnimation.AddBitmap(IDB_ATTACK, RGB(255, 255, 255));
	}
	void CRole::OnMove()
	{
		...
		if (isMovingUp && y > air) {
			y -= JDSTEP_SIZE;
			if (isMovingLeft && x - STEP_SIZE > -20) x -= STEP_SIZE;
			if (isMovingRight && x - STEP_SIZE < 617) x += STEP_SIZE;
			if (y == air) {
				SetMovingUp(false);
				SetMovingDown(true);
			}
	...
	void CRole::OnShow()
	{
		...
		if (isAttacking) {
			attackAnimation.SetTopLeft(x, y);
	...
	void CRole::bulletMoving(bool flag)
	{
		isbulletMoving = flag;
	}
